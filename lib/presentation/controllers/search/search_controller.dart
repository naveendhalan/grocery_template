...
class SearchController extends GetxController {
  ...
  final List<ProductEntity> _allProducts = [
    ...
    const ProductEntity(
      ...
      price: 55,
      mrp: 70,
      description: 'Nutritious broccoli packed with vitamins.',
      inStock: true,
    ),
    const ProductEntity(
      id: 'p7',
      ...
      switch (filter.type) {
        case FilterType.category:
          if (filter.value != null) {
            results = results
                .where(
                  (product) => _productCategory[product.id] == filter.value,
                )
                .toList();
          }
          break;
        case FilterType.price:
          if (filter.value != null) {
            ...
            if (priceRange.length == 2) {
              ...
              final min =
                  double.tryParse(filter.value!.replaceAll('+', '')) ?? 0;
              results = results
                  .where((product) => product.price >= min)
                  .toList();
            }
          }
          break;
